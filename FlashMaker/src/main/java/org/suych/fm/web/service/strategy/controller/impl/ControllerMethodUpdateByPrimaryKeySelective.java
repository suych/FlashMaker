...
@Component(ConstantStrategyComponentName.CONTROLLER_UPDATE_BY_PRIMARYKEY_SELECTIVE)
public class ControllerMethodUpdateByPrimaryKeySelective implements IControllerMethod {

	@Override
	public MethodStructure assemble() {
		MethodStructure result = new MethodStructure();
		// 注解
		List<AnnotationStructure> methodAnnotation = assembleMethodAnnotation(
				ConstantMethodName.UPDATE_BY_PRIMARYKEY_SELECTIVE);
		// 返回值
		String returnValue = STRING;
		// 方法名
		String methodName = ConstantMethodName.UPDATE_BY_PRIMARYKEY_SELECTIVE;
		// 参数
		...
		methodBody.append(TAB + TAB + BaseInfo.getServiceInterfaceFieldName() + POINT
				+ ConstantMethodName.UPDATE_BY_PRIMARYKEY_SELECTIVE + LEFT_BRACKET + BaseInfo.getDomainClassFieldName()
